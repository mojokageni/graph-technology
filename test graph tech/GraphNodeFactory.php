...
use craftitalk\craftitalkResponse;
...
class GraphNodeFactory
{
     ...
     */
    const BASE_GRAPH_NODE_CLASS = '\craftitalk\GraphNodes\GraphNode';

    /**
     * @const string The base graph edge class.
     */
    const BASE_GRAPH_EDGE_CLASS = '\craftitalk\GraphNodes\GraphEdge';

    /**
     * @const string The graph object prefix.
     */
    const BASE_GRAPH_OBJECT_PREFIX = '\craftitalk\GraphNodes\\';

    /**
     * @var craftitalkResponse The response entity from Graph.
     */
    protected $response;

    /**
     * @var array The decoded body of the craftitalkResponse entity from Graph.
     */
     ...
     *
     * @param craftitalkResponse $response The response entity from Graph.
     */
    public function __construct(craftitalkResponse $response)
    {
    ...
    public function validateResponseAsArray()
    {
        if (!is_array($this->decodedBody)) {
            throw new craftitalkSDKException('Unable to get response from Graph as array.', 620);
        }
    }
    ...
    public function validateResponseCastableAsGraphNode()
    {
        if (isset($this->decodedBody['data']) && static::isCastableAsGraphEdge($this->decodedBody['data'])) {
            throw new craftitalkSDKException(
    ...
    {
        if (!isset($data['data'])) {
            throw new craftitalkSDKException('Cannot cast data to GraphEdge. Expected a "data" key.', 620);
        }
    ...
    public static function validateSubclass($subclassName)
    {
        ...
        throw new craftitalkSDKException('The given subclass "' . $subclassName . '" is not valid. Cannot cast to an object that is not a GraphNode subclass.', 620);
    }
}
